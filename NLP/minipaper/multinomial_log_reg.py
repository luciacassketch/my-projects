...
class MLR():
    ...
    def train(self, lr=0.1):
        ...
        the outputs_dict, from which the model calculates the weights' sum for every tag. The weights are passed through
        a softmax activation function. The prediction is the tag with the highest probability. The model updates the 
        weights if the prediction was wrong via gradient descent.
        ...
        for epoch in range(1):
            for line in file:
                ...
                ## get the weight sum            
                score_vector = [sum(self.weights[function] for function in outputs_dict[combo]) for combo in outputs_dict]
                score_vector = numpy.array(score_vector)
                
                exp_values = numpy.exp(score_vector)
                exp_values_sum = numpy.sum(exp_values)
                softmax_output = exp_values/exp_values_sum

                gold_tag = line[2]
                gold_index = list(outputs_dict.keys()).index(gold_tag)
                p_gold = softmax_output[gold_index]
                cross_entropy_loss = -math.log(p_gold)

                predicted_probs = dict(zip(outputs_dict.keys(), softmax_output))

                for combo in outputs_dict:
                    y = 1 if combo == gold_tag else 0
                    p = predicted_probs[combo]
                    for f_n in outputs_dict[combo]:
                        gradient = (p - y)  # should be (p - y) * x but x = 1
                        self.weights[f_n] -= lr * gradient
    ...
    def test(self, testpath):
        ...
        way as train() does, and then retrieves the output with the highest probabilty.

        Args:
            testpath: path to test file.
        Return:
            accuracy: accuracy statistics;
            preds: array of 0 and 1 representing wrong and correct predictions.
        '''
        file = fileReader(testpath)
        tot = 0     # counter for total test lines
        correct = 0     # counter for correct predictions
        preds = []      # list of all predictions (0 if wrong and 1 if true), needed for boot-strap test in miinpaper.py
        ...
        for line in file:
            ...
            predicted_probs = dict(zip(outputs_dict.keys(), softmax_output))
            predicted = max(predicted_probs.keys(), key = lambda x: predicted_probs[x])
...
def main():
    ...
    model.train(lr=0.2)
    accuracy, _ = model.test("itatest.txt")
